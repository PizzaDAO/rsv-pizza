...
const InstagramIcon: React.FC<{ size: number; className?: string }> = ({ size, className }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" className={className}>
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      setFormData({
        ...
        brandInstagram: result.sponsor.brandInstagram || '',
        brandDescription: result.sponsor.brandDescription || '',
        ...
        <form onSubmit={handleSubmit} className="bg-[#16213e] rounded-xl border border-white/10 p-6 space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              />
              <IconInput
                customIcon={<InstagramIcon size={20} className="absolute left-3 top-1/2 -translate-y-1/2 text-theme-text-muted pointer-events-none" />}
                type="text"
                value={formData.brandInstagram || ''}
                onChange={e => handleChange('brandInstagram', e.target.value)}
                placeholder="Brand Instagram Handle"
              />
            </div>
            <IconInput
              icon={FileText}
