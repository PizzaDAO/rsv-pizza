...
export function PageViewStats({ stats }: PageViewStatsProps) {
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-lg font-semibold text-theme-text">{t('report.pageViews')}</h3>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-theme-surface rounded-xl p-4 border border-theme-stroke">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs text-theme-text-secondary">{t('report.totalViews')}</span>
          </div>
        ...
        <div className="bg-theme-surface rounded-xl p-4 border border-theme-stroke">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs text-theme-text-secondary">{t('report.uniqueVisitors')}</span>
          </div>
          <div className="text-2xl font-bold text-theme-text">
      ...
      {stats.dailyViews.length > 0 && (
        <div>
          <h4 className="text-sm font-medium text-theme-text mb-3">{t('report.dailyViewsLast30')}</h4>
          <div className="bg-theme-surface rounded-xl p-4 border border-theme-stroke">
                  ...
                  >
                    ...
                    <div className="absolute bottom-full mb-2 hidden group-hover:block z-10 pointer-events-none">
                      <div className="bg-black/90 border border-theme-stroke-hover rounded-lg px-3 py-2 text-xs whitespace-nowrap">
                        ...
                        <div className="text-theme-text">{day.total} {t('report.views')}</div>
                        <div className="text-theme-text-secondary">{day.unique} {t('report.unique')}</div>
                      </div>
                    </div>
      ...
      {stats.topReferrers.length > 0 && (
        <div>
          <h4 className="text-sm font-medium text-theme-text mb-3">{t('report.topReferrers')}</h4>
      ...
      {stats.totalViews === 0 && (
        <div className="text-center py-6">
          ...
          <p className="text-theme-text-muted text-sm">{t('report.noPageViewsYet')}</p>
          <p className="text-theme-text-faint text-xs mt-1">{t('report.pageViewsTracked')}</p>
        </div>
      )}
    </div>
  );
