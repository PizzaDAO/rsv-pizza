/**
 * calzone-58481: server-side catalog of host apps that can be linked from a
 * Telegram broadcast via the {appLink} token. The URL segment is the app's
 * `tab` value (the host app URL is /host/:inviteCode/:tab), NOT its id.
 *
 ...
 * `appTab` so a literal {appLink} token can never ship to a group.
 */
