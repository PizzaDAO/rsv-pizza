  ...
  try {
    ...
    for (const kit of filtered) {
      const row = [
        ...
        escapeCSV(kit.party.underbossStatus === 'approved' ? 'Approved' : kit.party.underbossStatus === 'rejected' ? 'Rejected' : 'Pending'),
...
router.get('/kits', requireAuth, requireShippingAuth, async (req: ShippingRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const formattedKits = filtered.map(kit => ({
      ...
      underbossStatus: kit.party.underbossStatus || 'pending',
  ...
  try {
    ...
    res.json({
      kit: {
        ...
        underbossStatus: updatedKit.party.underbossStatus || 'pending',
